...
Sub Delete_CPV_Group_Automation()
'On Error GoTo Surya
...
driver.wait 100
'-----------------------------------------------------------'-----------------------------------------------------------
Sheets("Delete_CPV_Filed").Activate
Dim i, lr As Long
...
For Each row In tbl.FindElementsByXPath("./tr")
        ...
        If (refftype.Text = "CPV GROUP ID") Then
           icon.Click
            driver.FindElementByLinkText("Delete").Click              ' click on update dropdown
...
'---------------------------------------------------------------------------------------------------
                driver.FindElementByXPath("//button[@class ='delete']").Click
                 Range("C" & i).Value = "Record Deleted"       ' click on update button
                logFile.WriteLine Now() & "----Record Deleted----"
                'Range("E" & i).Value = GetPropValue(driver)
                driver.wait 300
            Else
                Range("C" & i).Value = "Record Not Deleted"
                logFile.WriteLine Now() & "----Record Not Deleted----"
